...
const DetailProduct = () => {
    ...
    useEffect(() => {
        const fetchProductDetail = async () => {
            setLoading(true);
            try {
                const response = await fetch(`http://localhost:3001/api/v1/products/detail/${productId}`);
                const data = await response.json();
                if (data.code === 200) {
                    setProduct(data.data); // Cập nhật dữ liệu sản phẩm
                } else {
        ...
        <div className="container my-4" style={{ maxWidth: '1140px' }}>
            <div className="row">
                ...
                <div className="col-md-4">
                    <div className="image-gallery" style={{ height: '550px', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
                        <img
                    ...
                    <div className="thumbnail-gallery mt-2 d-flex justify-content-center">
                        {product.images && product.images.map((image, index) => (
                            <img
                                ...
                                style={{ cursor: 'pointer', width: '60px', height: '60px', margin: '0 5px', border: currentImageIndex === index ? '2px solid #007bff' : '2px solid transparent' }}
                            />
                        ))}
                    </div>
                </div>
                    ...
                    <div className="product-info">
                        ...
                        <div className="d-flex align-items-center">
                            <div className="rating" style={{ marginRight: '30px' }}>
                                ⭐ {product.rating || 5} {/* Hiển thị sao giả định nếu không có rating */}
                            </div>
                            <div className="review-info d-flex align-items-center">
                                <span style={{ marginRight: '-5px' }}>{product.reviews ? product.reviews.length : 0}</span>
                                <button className="btn btn-link" onClick={handleScrollToReviews}>Đánh giá</button>
                            </div>
                        </div>

                        <div className="store-name mb-2">Cửa hàng: {product.storeName}</div>
                        <div className="product-id mb-2">Mã sản phẩm: {product.productId}</div>
                        <div className="quantity mb-2">Số lượng còn: {product.stock_item ? product.stock_item.qty : 0}</div>
                        <div className="quantity-sold mb-2">Đã bán: {product.quantity_sold ? product.quantity_sold.text : '0'}</div>
                        <hr />

                        <div className="prices mt-2">
                            <span className="new-price h4">{product.price} VNĐ</span>
                            <span className="old-price text-muted ml-2">{product.original_price} VNĐ</span>
                        </div>

                        { product.discount_rate > 0 && (
                            <div className="discount mt-2 p-3">
                                <strong>
                                    Mã Giảm Giá Của Shop: <span className="discount-rate">{`Giảm ${product.discount_rate}%`}</span>
                                </strong>
                            </div>
                        )}
                        ...
                        <div className="quantity mt-3">
                            ...
                            <div className="d-flex align-items-center mt-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        const value = e.target.value === "" ? "" : Math.max(1, Math.min(product.stock_item.qty || 1000, parseInt(e.target.value) || 1));
                                        setQuantity(value);
                                    }}
                                    onBlur={() => {
                                        if (quantity === "") {
                                ...
                                />
                                <FaRegPlusSquare
                                    onClick={() => setQuantity(quantity < (product.stock_item.qty || 1000) ? quantity + 1 : (product.stock_item.qty || 1000))}
                                    style={{ cursor: 'pointer', fontSize: '20px' }}
                                />
                            </div>
                        </div>
